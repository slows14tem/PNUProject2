...
public class TokenProvider {// JWT 토큰에 관련된 암호화, 복호화, 검증 로직은 다 이곳에서 이루어집니다.
	...
	public TokenDto generateTokenDto(Authentication authentication) { // 유저 정보를 넘겨받아서 Access Token 과 Refresh Token 을
																		// 생성합니다.
		// 권한들 가져오기
		...
		// Refresh Token 생성
		String refreshToken = Jwts.builder().setExpiration(new Date(now + REFRESH_TOKEN_EXPIRE_TIME))// 만료시간설정
				.signWith(key, SignatureAlgorithm.HS512)// signWith() 메서드를 사용하여 JWT에 서명 이때, key 변수는 서명에 사용할 비밀키를,
														// SignatureAlgorithm.HS512는 서명 알고리즘을 지정합니다.
		...
		return TokenDto.builder() // TokenDto 객체를 생성하여 생성된 JWT와 함께 반환합니다.
				...
				.accessToken(accessToken).accessTokenExpiresIn(accessTokenExpiresIn.getTime())
				.refreshToken(refreshToken) // 생성된 access token, 만료 시간, 그리고 refreshToken을 포함합니다
				.build(); // 최종적으로, return 키워드를 사용하여 TokenDto 객체를 반환합니다.
	}

	public Authentication getAuthentication(String accessToken) { // Access Token 에만 유저 정보를 담기 때문에 명시적으로 accessToken 을
																	// 파라미터로 받게 했습니다.
	...
	public boolean validateToken(String token) { // 토큰 정보를 검증
		try {
			Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(token); // parseClaims 메소드는 만료된 토큰이어도 정보를 꺼내기
																					// 위해서 따로 분리
			return true;
