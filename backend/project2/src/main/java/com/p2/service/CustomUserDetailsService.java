...
public class CustomUserDetailsService implements UserDetailsService {
	...
	@Override
	@Transactional
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException { // 여기서 넘겨받은 UserDetails 와
																								// Authentication 의
																								// 패스워드를 비교하고 검증하는 로직을
																								// 처리합니다.
	...
	private UserDetails createUserDetails(MemberVO member) {
		...
		return new User(String.valueOf(member.getId()), member.getPass(), Collections.singleton(grantedAuthority));
	}
}
